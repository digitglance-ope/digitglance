...
export default function ReportsPage() {
  ...
  const [activeTab, setActiveTab] = useState<'summary' | 'vat' | 'receivable' | 'payable'>('summary')
  ...
  // Accounts receivable: group outstanding and partial invoices by customer
  ...
  const arByCustomer = allOutstanding.reduce<Record<string, ARCustomer>>((acc, inv) => {
    ...
    if (!acc[name]) {
      acc[name] = { name, totalOwed: 0, invoiceCount: 0, oldestDue: inv.due_date }
    }
  ...
  function exportARCSV() {
    ...
    const a = document.createElement('a')
    a.href = url
    a.download = `accounts-receivable-${startDate}-to-${endDate}.csv`
    a.click()
    URL.revokeObjectURL(url)
  }
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex">
      ...
      <main className="ml-64 flex-1 p-8">
        ...
        {/* Filters - only show for invoice tabs */}
        {(activeTab === 'summary' || activeTab === 'vat' || activeTab === 'receivable') && (
          ...
          <>
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
              {[
                ...
                { label: 'Total VAT', value: fmt(totalVAT), color: 'text-teal-600' },
              ].map(card => (
        ...
        <div className="bg-white border border-slate-200 rounded-xl overflow-hidden">
          {loading ? (
            ...
            invoices.filter(i => i.vat_amount > 0).length === 0 ? (
              <div className="p-12 text-center text-slate-400 text-sm">No VATable invoices found for this period.</div>
            ) : (
              <table className="w-full">
                <thead>
                  <tr className="border-b border-slate-200 bg-slate-50">
                  ...
                  {arList.map(c => {
                    ...
                    return (
                      <tr key={c.name} className="hover:bg-slate-50">
                        ...
                        <td className="px-5 py-3">
                          {isOverdue ? (
                            <span className="inline-flex px-2 py-1 rounded-full text-xs font-semibold bg-red-100 text-red-700">Overdue</span>
                          ) : (
                            <span className="inline-flex px-2 py-1 rounded-full text-xs font-semibold bg-green-100 text-green-700">Current</span>
                          )}
                        </td>
